...
static void tcp_update_skb_after_send_pacing(struct sock *sk, struct sk_buff *skb,
				      u64 prior_wstamp)
{
	struct tcp_sock *tp = tcp_sk(sk);
...
void tcp_update_skb_after_send(struct tcp_sock *tp,
			       struct sk_buff *skb)
{
...
#endif
{
	...
	while ((skb = tcp_send_head(sk))) {
		...
		if (unlikely(tp->repair) && tp->repair_queue == TCP_SEND_QUEUE) {
			...
			tcp_update_skb_after_send_pacing(sk, skb, tp->tcp_wstamp_ns);
